...
## 🎨 인스타 이미지 스타일 (2026-08-27 지시 — 블로그 썸네일 규칙과 별개)
인스타용 카드·이미지는 **극사실주의 + 최고 퀄리티 + 시선을 확 끄는 자극적 구도**로 만든다. 챗지피티(이메일 루트) 프롬프트에 아래를 반드시 녹인다:
- **극사실주의(hyper-realistic / photorealistic)** — 실사 사진급. 일러스트·플랫 금지.
- **광고급 고퀄리티** — 8K, 시네마틱 조명, 얕은 심도, 디테일 극대화, 프리미엄 광고 비주얼.
- **스크롤을 멈추게 하는 자극적 훅** — 강한 대비·클로즈업·극적인 순간·과감한 구도로 대중의 시선을 확 끈다(피드에서 튀게). 팔레트에 얽매이지 말고 임팩트 우선.
- **단, 텍스트·수치·주장은 정확하게** — 비주얼은 자극적이되 내용(면세 기준·규정)은 과장·허위 금지. 브랜드 로고·상표는 넣지 않는다. 한글 텍스트는 오타 없이 정확히.
- 비율: 피드=1:1(1080) 또는 4:5, 스토리=9:16.
(블로그 썸네일/본문이미지의 '파스텔·톤 다양화' 규칙은 블로그용이고, 인스타는 이 규칙을 따른다.)
